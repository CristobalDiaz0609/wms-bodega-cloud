...
st.set_page_config(page_title="WMS Bodega Inteligente", layout="wide", page_icon="📦")
...
if menu == "🗺️ Mapa 2D & Estado":
    ...
    if not df_mapa.empty:
        ...
        fig.update_layout(xaxis=dict(tickmode='linear', dtick=1), yaxis=dict(tickmode='linear', dtick=1))
        st.plotly_chart(fig, use_container_width=True)
        
        st.subheader("Detalle de Ubicaciones")
        st.dataframe(df_mapa[["id_ubicacion", "estado", "sku", "producto", "cantidad", "capacidad", "Ocupacion_%"]], use_container_width=True)
...
# ---------------------------------------------------------
    ...
    df_libres = obtener_df("SELECT id_ubicacion FROM ubicaciones WHERE estado = 'Libre'")
    
    if df_prods.empty or df_libres.empty:
        st.warning("Asegúrate de tener productos registrados y casillas libres disponibles.")
    else:
        with st.form("form_ingreso"):
            sku_sel = st.selectbox("Seleccionar Producto (SKU)", df_prods["sku"] + " - " + df_prods["nombre"])
            sku_limpio = sku_sel.split(" - ")[0]
            
            cap_max = df_prods[df_prods["sku"] == sku_limpio]["capacidad_por_casilla"].values[0]
            st.info(f"Capacidad máxima permitida por casilla para este SKU: {cap_max} unidades.")
            
            ubicacion_sel = st.selectbox("Seleccionar Casilla Disponible", df_libres["id_ubicacion"])
            cantidad_ingreso = st.number_input("Cantidad a Ingresar", min_value=1, max_value=int(cap_max), value=1)
            ...
            if btn_ingresar:
                ...
                ejecutar_query("UPDATE ubicaciones SET estado = 'Ocupado' WHERE id_ubicacion = %s", (ubicacion_sel,))
                ejecutar_query(
                ...
                st.success(f"✅ Se ingresaron {cantidad_ingreso} unidades de {sku_limpio} en la casilla {ubicacion_sel}.")
                st.rerun()
...
# ---------------------------------------------------------
    ...
    if df_inv.empty:
        ...
        sku_despacho = st.selectbox("Seleccionar Producto a Despachar", df_inv["sku"] + " - " + df_inv["nombre"])
        sku_clean = sku_despacho.split(" - ")[0]
        
        max_disponible = int(df_inv[df_inv["sku"] == sku_clean]["total_disponible"].values[0])
        cant_solicitada = st.number_input(f"Cantidad a Solicitar (Máx: {max_disponible})", min_value=1, max_value=max_disponible, value=1)
        ...
        if st.button("Generar Hoja de Ruta / Ejecutar Picking"):
            ...
            st.subheader("📋 Hoja de Ruta para Operador:")
            st.table(pd.DataFrame(hoja_ruta))
            st.rerun()
...
# ---------------------------------------------------------
    ...
    total_casillas = obtener_df("SELECT COUNT(*) as t FROM ubicaciones")["t"].values[0]
    casillas_ocupadas = obtener_df("SELECT COUNT(*) as t FROM ubicaciones WHERE estado = 'Ocupado'")["t"].values[0]
    ...
    stock_actual = obtener_df("SELECT COALESCE(SUM(cantidad), 0) as t FROM inventario")["t"].values[0]
    
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Ocupación de Casillas", f"{(casillas_ocupadas/total_casillas)*100:.1f}%", f"{casillas_ocupadas}/{total_casillas} Módulos")
    col2.metric("Ocupación Volumétrica", f"{(stock_actual/cap_tot)*100:.1f}%", f"{stock_actual} Unidades")
    ...
    df_kardex_exp = obtener_df("SELECT * FROM historial_movimientos ORDER BY fecha_hora DESC")
...
# ---------------------------------------------------------
# 6. GENERADOR DE ETIQUETAS QR (Último Módulo Agregado)
# ---------------------------------------------------------
elif menu == "🏷️ Generador de Etiquetas QR":
    st.header("Generador e Impresión de Etiquetas QR")
    st.write("Genera e imprime códigos QR para identificar casillas físicamente o etiquetar paletas/productos.")
    
    opcion_qr = st.radio("¿Qué tipo de etiqueta deseas generar?", ["Ubicación / Casilla", "Producto / SKU"])
    
    if opcion_qr == "Ubicación / Casilla":
        df_ubis = obtener_df("SELECT id_ubicacion FROM ubicaciones")
        if not df_ubis.empty:
            ubi_sel = st.selectbox("Seleccionar Casilla para Etiquetar", df_ubis["id_ubicacion"])
            ...
            if st.button("Generar Código QR de Casilla"):
                ...
                st.image(buf.getvalue(), caption=f"Etiqueta QR para Casilla: {ubi_sel}", width=250)
                st.download_button(
        ...
        if not df_prods.empty:
            prod_sel = st.selectbox("Seleccionar Producto para Etiquetar", df_prods["sku"] + " - " + df_prods["nombre"])
            ...
            if st.button("Generar Código QR de Producto"):
                ...
                st.image(buf.getvalue(), caption=f"Etiqueta QR para SKU: {sku_qr}", width=250)
                st.download_button(
